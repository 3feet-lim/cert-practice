...
export function describeRepositoryContractSuite(
  ...
  describe(
    `${adapterName} aggregate repository contract`,
    ...
    () => {
      ...
      it("enforces owner scope and maintains one active practice slot", async () => {
        ...
        await unitOfWork.transaction(async (repos) => {
          expect(
            await repos.practice.findActiveOwned(ids.userA, "cert-a"),
          ).toMatchObject({
            id: ids.practiceNew,
          });
      ...
      it("persists exact fractions, locks the first answer, and hides expired results", async () => {
        ...
        await unitOfWork.transaction(async (repos) => {
          expect(
            await repos.practice.getCompletedOwned(ids.userB, ids.result, later(2)),
          ).toBeNull();
          expect(
            await repos.practice.getCompletedOwned(ids.userA, ids.result, later(2)),
          ).toMatchObject({
            id: ids.result,
          });
          expect(
            await repos.practice.getCompletedOwned(ids.userA, ids.result, later(169)),
          ).toBeNull();
        });
      });
      ...
      it("finalizes each exam once and returns its immutable attempt on replay", async () => {
        ...
        const first = await unitOfWork.transaction((repos) =>
          repos.exams.finalizeOnce(command),
        );
        const replay = await unitOfWork.transaction((repos) =>
