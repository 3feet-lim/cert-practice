    ...
    return {
      ...
      generationSource: async (certificationId) => {
        ...
        return {
          ...
          questions: full.questions.map(
            ({ id, revisionId, certificationId: cid, domainId }) => ({
              id,
              revisionId,
              certificationId: cid,
              domainId,
      ...
      fullGenerationSource: async (certificationId) =>
        loadFullSource(this.db, certificationId),
    };
  }
  ...
  private practiceRepository(): PracticeRepository {
    return {
      ...
      listActiveOwned: async (userId) => {
        ...
        const sessions = await Promise.all(
          headers.rows.map((row) =>
            loadPractice(this.db, "user_id = $1 AND id = $2", [userId, row.id]),
          ),
        );
        return sessions.filter(
          (session): session is PracticeSession => session !== null,
        );
      },
      ...
      replaceAtomically: async (input) => {
        ...
        const certificationId = await activeCertificationId(
          this.db,
          input.certificationKey,
        );
      ...
      saveState: async (command) => {
        ...
        const item = await this.db.query(
          ...
          [
            command.sessionId,
            command.questionId,
            json(command.selectedChoiceIds),
            command.flagged,
          ],
        );
        if (item.rowCount !== 1) return null;
        return loadPractice(this.db, "user_id = $1 AND id = $2", [
          command.userId,
          command.sessionId,
        ]);
      },
  ...
  private examRepository(): ExamRepository {
    return {
      createWithSnapshots: async (input) => {
        ...
        if (existing.rows[0])
          return required(
            await loadExam(this.db, "id = $1", [existing.rows[0].id]),
            "exam",
          );
        const certificationId = await activeCertificationId(
          this.db,
        ...
        await this.db.query(
          ...
          [
            ...
            certificationId,
            input.certificationKey,
            json(certificationSnapshot(input.questions)),
            input.startRequestKey,
            input.currentIndex,
            input.startedAt,
            input.expiresAt,
          ],
      ...
      getOwned: async (userId, sessionId) =>
        loadExam(this.db, "user_id = $1 AND id = $2", [userId, sessionId]),
      listExpiredOwned: async (userId, now) => {
        ...
        const sessions = await Promise.all(
          headers.rows.map((row) =>
            loadExam(this.db, "user_id = $1 AND id = $2", [userId, row.id]),
          ),
        );
      ...
      saveBeforeExpiry: async (command) => {
        const current = await loadExam(this.db, "user_id = $1 AND id = $2", [
          command.userId,
          command.sessionId,
        ]);
        if (!current) return null;
        const question = current.questions.map((item) =>
  ...
  private historyRepository(): HistoryRepository {
    return {
      getAttemptOwned: async (userId, attemptId) =>
        loadAttempt(this.db, "user_id = $1 AND id = $2", [userId, attemptId]),
      listAttempts: async (userId) => {
        const rows = await this.db.query<{ id: string }>(
          `SELECT id FROM attempts WHERE user_id = $1 ORDER BY submitted_at DESC, id ASC`,
          [userId],
        );
        return Promise.all(
      ...
      listPublicAttempts: async (certificationId) => {
        ...
        for (const row of rows.rows) {
          const [user, attempt] = await Promise.all([
            ...
            loadAttempt(this.db, "user_id = $1 AND id = $2", [
              row.user_id,
              row.attempt_id,
            ]),
          ]);
...
const userColumns =
  "id, google_sub, display_name, email, role, approval_status, score_public, first_login_at, approved_at, version";
...
type ExamRow = QueryResultRow & {
  id: string;
  user_id: string;
  certification_key: string;
  start_request_key: string;
...
type QuestionRow = QueryResultRow & {
  ...
  snapshot_content: JsonValue | string;
  selected_choice_ids: JsonValue | string;
  final_choice_ids: JsonValue | string | null;
  earned_numerator: string | bigint | null;
  earned_denominator: string | bigint | null;
  flagged: boolean;
...
type ResultRow = QueryResultRow & {
  ...
  user_id: string;
  raw_numerator: string | bigint;
  raw_denominator: string | bigint;
  accuracy_numerator: string | bigint;
  accuracy_denominator: string | bigint;
...
type AttemptRow = QueryResultRow & {
  ...
  accuracy_numerator: string | bigint;
  accuracy_denominator: string | bigint;
  threshold_numerator: string | bigint;
  threshold_denominator: string | bigint;
  passed: boolean;
  reference_1000: number;
  started_at: Date | string;
  expires_at: Date | string;
...
function mapUser(row: UserRow): UserProfile {
  return {
    ...
    scorePublic: row.score_public,
    firstLoginAt: date(row.first_login_at),
    approvedAt: row.approved_at ? date(row.approved_at) : null,
    version: bigint(row.version),
  };
}
function mapRevision(row: RevisionRow): CatalogRevision {
  return {
    id: row.id,
...
function mapQuestion(row: QuestionRow): PersistedQuestionSnapshot {
  return {
    ...
    content: object(row.snapshot_content),
    selectedChoiceIds: strings(row.selected_choice_ids),
    finalChoiceIds:
      row.final_choice_ids === null ? null : strings(row.final_choice_ids),
    earnedScore:
      row.earned_numerator === null || row.earned_denominator === null
        ? null
        : Fraction.of(bigint(row.earned_numerator), bigint(row.earned_denominator)),
    flagged: row.flagged,
    ...(row.saved_at ? { savedAt: date(row.saved_at) } : {}),
    version: bigint(row.version),
  };
}
function mapResult(row: ResultRow): CompletedPracticeResult {
  return {
    id: row.id,
    sourcePracticeSessionId: row.source_practice_session_id,
    userId: row.user_id,
    rawScore: Fraction.of(bigint(row.raw_numerator), bigint(row.raw_denominator)),
    accuracyRate: Fraction.of(
      bigint(row.accuracy_numerator),
      bigint(row.accuracy_denominator),
    ),
...
async function loadPractice(
  ...
  const header = await db.query<SessionRow>(
    `SELECT id, user_id, certification_key, status, current_index, result_id, created_at, completed_at, version FROM practice_sessions WHERE ${condition} ORDER BY created_at ASC LIMIT 1`,
    values,
  );
  const row = header.rows[0];
  if (!row) return null;
  const questions = await db.query<QuestionRow>(
    `SELECT id, display_index, snapshot_content, selected_choice_ids, final_choice_ids, earned_numerator, earned_denominator, flagged, version FROM practice_session_questions WHERE practice_session_id = $1 ORDER BY display_index ASC`,
    [row.id],
  );
  return {
    id: row.id,
...
async function loadExam(
  ...
  const header = await db.query<ExamRow>(
    `SELECT id, user_id, certification_key, start_request_key, status, current_index, started_at, expires_at, attempt_id, version FROM exam_sessions WHERE ${condition} ORDER BY started_at ASC LIMIT 1`,
    values,
  );
  const row = header.rows[0];
  if (!row) return null;
  const questions = await db.query<QuestionRow>(
    `SELECT id, display_index, snapshot_content, selected_choice_ids, NULL::jsonb AS final_choice_ids, NULL::bigint AS earned_numerator, NULL::bigint AS earned_denominator, flagged, saved_at, version FROM exam_session_questions WHERE exam_session_id = $1 ORDER BY display_index ASC`,
    [row.id],
  );
  return {
    id: row.id,
    userId: row.user_id,
    certificationKey: row.certification_key,
    startRequestKey: row.start_request_key,
    status: row.status,
    currentIndex: row.current_index,
...
async function loadAttempt(
  ...
  const header = await db.query<AttemptRow>(
    `SELECT id, exam_session_id, user_id, certification_key, raw_numerator, raw_denominator, accuracy_numerator, accuracy_denominator, threshold_numerator, threshold_denominator, passed, reference_1000, started_at, expires_at, submitted_at, submission_reason FROM attempts WHERE ${condition} LIMIT 1`,
    values,
  );
  const row = header.rows[0];
  if (!row) return null;
  const items = await db.query<QuestionRow>(
    `SELECT id, display_index, snapshot_content, selected_choice_ids, selected_choice_ids AS final_choice_ids, earned_numerator, earned_denominator, false AS flagged, 0::bigint AS version FROM attempt_items WHERE attempt_id = $1 ORDER BY display_index ASC`,
    [row.id],
  );
  return {
    id: row.id,
    ...
    passThreshold: Fraction.of(
      bigint(row.threshold_numerator),
      bigint(row.threshold_denominator),
    ),
    passed: row.passed,
    reference1000Score: row.reference_1000,
    startedAt: date(row.started_at),
    expiresAt: date(row.expires_at),
    submittedAt: date(row.submitted_at),
    submissionReason: row.submission_reason,
    items: items.rows.map(mapQuestion),
  };
}

async function insertPracticeQuestions(
  db: Queryable,
  sessionId: string,
  ...
  for (const item of questions)
    await db.query(
      `INSERT INTO practice_session_questions (id, practice_session_id, display_index, snapshot_content, selected_choice_ids, final_choice_ids, earned_numerator, earned_denominator, flagged, updated_at, version) VALUES ($1,$2,$3,$4::jsonb,$5::jsonb,$6::jsonb,$7,$8,$9,$10,$11)`,
      [
        item.id,
        sessionId,
        item.displayIndex,
        json(item.content),
        json(item.selectedChoiceIds),
        item.finalChoiceIds === null ? null : json(item.finalChoiceIds),
        item.earnedScore?.numerator.toString() ?? null,
        item.earnedScore?.denominator.toString() ?? null,
        item.flagged,
        now,
...
async function insertExamQuestions(
  ...
  for (const item of questions)
    await db.query(
      `INSERT INTO exam_session_questions (id, exam_session_id, display_index, snapshot_content, selected_choice_ids, flagged, saved_at, version) VALUES ($1,$2,$3,$4::jsonb,$5::jsonb,$6,$7,$8)`,
      [
        item.id,
        sessionId,
        item.displayIndex,
...
async function submitPractice(db: Queryable, command: SubmitPracticeAnswer) {
  const current = await loadPractice(db, "user_id = $1 AND id = $2", [
    command.userId,
    command.sessionId,
  ]);
  if (!current) return null;
  ...
  if (existing.finalChoiceIds)
    return {
      session: current,
      result: current.resultId ? await completedBySession(db, command.sessionId) : null,
      firstSubmission: false,
    };
  const parent = await db.query(
    `UPDATE practice_sessions SET version = version + 1, updated_at = now() WHERE id = $1 AND user_id = $2 AND status = 'active' AND version = $3`,
    [command.sessionId, command.userId, command.expectedVersion.toString()],
  );
  if (parent.rowCount !== 1) return null;
  const item = await db.query(
    `UPDATE practice_session_questions SET selected_choice_ids = $3::jsonb, final_choice_ids = $3::jsonb, earned_numerator = $4, earned_denominator = $5, submitted_at = now(), updated_at = now(), version = version + 1 WHERE practice_session_id = $1 AND id = $2 AND final_choice_ids IS NULL`,
    [
      command.sessionId,
      command.questionId,
      json(command.selectedChoiceIds),
      command.earnedScore.numerator.toString(),
      command.earnedScore.denominator.toString(),
    ],
  );
  if (item.rowCount !== 1) return null;
  const completed = current.questions.every(
    (question) =>
      question.id === command.questionId || question.finalChoiceIds !== null,
  );
  ...
  if (completed) {
    ...
    await db.query(
      `UPDATE practice_sessions SET status = 'completed', active_slot = NULL, result_id = $2, completed_at = $3 WHERE id = $1`,
      [command.sessionId, result.id, result.completedAt],
    );
  }
  return {
    session: required(
      await loadPractice(db, "user_id = $1 AND id = $2", [
        command.userId,
        command.sessionId,
      ]),
      "practice",
...
async function insertCompletedResult(
  db: Queryable,
  input: NewCompletedPracticeResult,
  session: PracticeSession,
): Promise<CompletedPracticeResult> {
  await db.query(
    ...
    [
      ...
      session.id,
      session.userId,
      json(certificationSnapshot(session.questions)),
      input.rawScore.numerator.toString(),
      input.rawScore.denominator.toString(),
      input.accuracyRate.numerator.toString(),
      input.accuracyRate.denominator.toString(),
      input.completedAt,
      input.expiresAt,
      json(input.payload),
    ],
  ...
  for (const item of items.rows) {
    if (
      item.final_choice_ids === null ||
      item.earned_numerator === null ||
      item.earned_denominator === null
    )
    ...
    await db.query(
      ...
      [
        ...
        input.id,
        item.display_index,
        json(object(item.snapshot_content)),
        json(strings(item.final_choice_ids)),
        item.earned_numerator.toString(),
...
async function completedBySession(
  db: Queryable,
  sessionId: string,
): Promise<CompletedPracticeResult | null> {
  const row = await db.query<ResultRow>(
    `SELECT id, source_practice_session_id, user_id, raw_numerator, raw_denominator, accuracy_numerator, accuracy_denominator, completed_at, expires_at, payload FROM completed_practice_results WHERE source_practice_session_id = $1`,
    [sessionId],
  );
  return row.rows[0] ? mapResult(row.rows[0]) : null;
}

async function replaceExamState(
  db: Queryable,
  userId: string,
  sessionId: string,
  expected: bigint,
  ...
  const parent = await db.query(
    `UPDATE exam_sessions SET current_index = $3, version = version + 1 WHERE id = $1 AND user_id = $2 AND status = 'active' AND expires_at > $4 AND version = $5`,
    [sessionId, userId, session.currentIndex, now, expected.toString()],
  );
  if (parent.rowCount !== 1) return null;
  for (const item of session.questions)
    await db.query(
      `UPDATE exam_session_questions SET selected_choice_ids = $3::jsonb, flagged = $4, saved_at = $5, version = version + 1 WHERE exam_session_id = $1 AND id = $2`,
      [
        sessionId,
        item.id,
        json(item.selectedChoiceIds),
  ...
  return loadExam(db, "user_id = $1 AND id = $2", [userId, sessionId]);
}
async function finalizeExam(
  db: Queryable,
  command: FinalizeExam,
): Promise<Attempt | null> {
  const current = await loadExam(db, "user_id = $1 AND id = $2", [
    command.userId,
    command.sessionId,
  ]);
  if (!current) return null;
  if (current.status === "submitted")
    return current.attemptId
      ? loadAttempt(db, "user_id = $1 AND id = $2", [command.userId, current.attemptId])
      : null;
  const transition = await db.query(
    `UPDATE exam_sessions SET status = 'submitted', attempt_id = $3, submitted_at = $4, version = version + 1 WHERE id = $1 AND user_id = $2 AND status = 'active'`,
    [command.sessionId, command.userId, command.id, command.submittedAt],
  );
  if (transition.rowCount !== 1) {
    const replay = await loadExam(db, "user_id = $1 AND id = $2", [
      command.userId,
      command.sessionId,
    ]);
    return replay?.attemptId
      ? loadAttempt(db, "user_id = $1 AND id = $2", [command.userId, replay.attemptId])
      : null;
  }
  const items = command.items ?? current.questions;
  await db.query(
    `INSERT INTO attempts (id, exam_session_id, user_id, certification_key, certification_snapshot, raw_numerator, raw_denominator, accuracy_numerator, accuracy_denominator, reference_1000, threshold_numerator, threshold_denominator, passed, domain_performance, started_at, expires_at, submitted_at, submission_reason) VALUES ($1,$2,$3,$4,$5::jsonb,$6,$7,$8,$9,$10,$11,$12,$13,'[]'::jsonb,$14,$15,$16,$17)`,
    [
      command.id,
      current.id,
      current.userId,
      current.certificationKey,
      json(certificationSnapshot(items)),
      command.rawScore.numerator.toString(),
      command.rawScore.denominator.toString(),
      command.accuracyRate.numerator.toString(),
      command.accuracyRate.denominator.toString(),
      command.reference1000Score,
      command.passThreshold.numerator.toString(),
      command.passThreshold.denominator.toString(),
      command.passed,
      current.startedAt,
      current.expiresAt,
      command.submittedAt,
  ...
  for (const item of items)
    await db.query(
      `INSERT INTO attempt_items (id, attempt_id, display_index, snapshot_content, selected_choice_ids, earned_numerator, earned_denominator) VALUES ($1,$2,$3,$4::jsonb,$5::jsonb,$6,$7)`,
      [
        item.id,
        command.id,
        item.displayIndex,
        json(item.content),
        json(item.selectedChoiceIds),
        (item.earnedScore ?? Fraction.of(0n)).numerator.toString(),
        (item.earnedScore ?? Fraction.of(0n)).denominator.toString(),
      ],
    );
  return required(
    await loadAttempt(db, "user_id = $1 AND id = $2", [command.userId, command.id]),
    "attempt",
  );
}

async function activeCertificationId(
  db: Queryable,
  key: string,
): Promise<string | null> {
  const result = await db.query<{ id: string }>(
    `SELECT certification.id FROM catalog_heads head JOIN certifications certification ON certification.revision_id = head.active_revision_id WHERE head.certification_key = $1 AND certification.external_key = $1`,
    [key],
  );
  return result.rows[0]?.id ?? null;
}
async function consumeValidation(
  db: Queryable,
  command: Pick<
    ImportCommitCommand,
    "validationId" | "actorUserId" | "contentHash" | "now"
  > & { tokenDigest?: string },
): Promise<boolean> {
  const token = command.tokenDigest ? " AND token_digest = $5" : "";
  const values = command.tokenDigest
    ? [
    ...
    : [command.validationId, command.actorUserId, command.contentHash, command.now];
  const result = await db.query(
    `UPDATE import_validations SET status = 'consumed', consumed_at = $4, version = version + 1 WHERE id = $1 AND actor_user_id = $2 AND content_hash = $3 AND status = 'validated' AND expires_at > $4${token} RETURNING id`,
    values,
  );
...
type RevisionCounts = {
  ...
  orphan_questions: number | string;
  orphan_choices: number | string;
};
async function readHead(
  db: Queryable,
  certificationKey: string,
): Promise<CatalogHead | null> {
  const result = await db.query<{
    active_revision_id: string;
    version: string | bigint;
  }>(
    `SELECT active_revision_id, version FROM catalog_heads WHERE certification_key = $1`,
    [certificationKey],
  );
  const row = result.rows[0];
  return row
    ? { revisionId: row.active_revision_id, version: bigint(row.version) }
    : null;
}
async function insertRevision(
  ...
  await db.query(
    `INSERT INTO catalog_revisions (id, certification_key, content_hash, imported_by, imported_at, status) VALUES ($1,$2,$3,$4,$5,$6)`,
    [
      revision.id,
      revision.certificationKey,
...
async function assertPersistedRevision(
  db: Queryable,
  revisionId: string,
  source: CatalogRevisionSource,
  generation: FullCatalogGenerationSource,
): Promise<void> {
  const result = await db.query<RevisionCounts>(
    `SELECT (SELECT count(*) FROM providers WHERE revision_id = $1) AS providers, (SELECT count(*) FROM certifications WHERE revision_id = $1) AS certifications, (SELECT count(*) FROM domains WHERE revision_id = $1) AS domains, (SELECT count(*) FROM questions WHERE revision_id = $1) AS questions, (SELECT count(*) FROM choices WHERE revision_id = $1) AS choices, (SELECT count(*) FROM questions question LEFT JOIN domains domain ON domain.id = question.domain_id AND domain.revision_id = question.revision_id AND domain.certification_id = question.certification_id WHERE question.revision_id = $1 AND domain.id IS NULL) AS orphan_questions, (SELECT count(*) FROM choices choice LEFT JOIN questions question ON question.id = choice.question_id AND question.revision_id = choice.revision_id WHERE choice.revision_id = $1 AND question.id IS NULL) AS orphan_choices`,
    [revisionId],
  );
  const row = required(result.rows[0], "persisted catalog revision");
  const expectedChoices = generation.questions.reduce(
    (total, question) => total + question.choices.length,
    0,
  );
  if (
    Number(row.providers) !== source.providers.length ||
    Number(row.certifications) !== source.certifications.length ||
    Number(row.domains) !== source.domains.length ||
    Number(row.questions) !== generation.questions.length ||
    Number(row.choices) !== expectedChoices ||
    Number(row.orphan_questions) !== 0 ||
    Number(row.orphan_choices) !== 0
  )
    throw new Error("Persisted import revision failed verification.");
}
async function activateRevision(
  ...
  await db.query(
    `UPDATE catalog_revisions SET status = CASE WHEN id = $2 THEN 'active' ELSE 'superseded' END WHERE certification_key = $1 AND (id = $2 OR status = 'active')`,
    [certificationKey, revisionId],
  );
}
async function switchHead(db: Queryable, key: string, revisionId: string, now: Date) {
  await db.query(
    `INSERT INTO catalog_heads (certification_key, active_revision_id, updated_at, version) VALUES ($1,$2,$3,0) ON CONFLICT (certification_key) DO UPDATE SET active_revision_id = EXCLUDED.active_revision_id, updated_at = EXCLUDED.updated_at, version = catalog_heads.version + 1`,
    [key, revisionId, now],
  );
}
async function switchHeadConditionally(
  ...
  revisionId: string,
  now: Date,
  expected: CatalogHead | null,
): Promise<void> {
  const result = expected
    ? await db.query(
        `UPDATE catalog_heads SET active_revision_id = $2, updated_at = $3, version = version + 1 WHERE certification_key = $1 AND active_revision_id = $4 AND version = $5 RETURNING certification_key`,
        [key, revisionId, now, expected.revisionId, expected.version.toString()],
      )
    : await db.query(
        `INSERT INTO catalog_heads (certification_key, active_revision_id, updated_at, version) VALUES ($1,$2,$3,0) ON CONFLICT (certification_key) DO NOTHING RETURNING certification_key`,
        [key, revisionId, now],
      );
  if (result.rowCount !== 1) throw new Error("Catalog head changed during import.");
}
function assertMaterialization(command: ImportCommitCommand) {
  const { revision, source, generation } = command.materialization;
  const sourceQuestionIds = new Set(source.questions.map((question) => question.id));
  const generationQuestionIds = new Set(
    generation.questions.map((question) => question.id),
  );
  const expectedCertification = source.certifications.find(
    (item) => item.id === generation.certification.id,
  );
  const sourceDomains = new Set(source.domains.map((domain) => domain.id));
  const valid =
    revision.certificationKey === source.certificationKey &&
    revision.contentHash === command.contentHash &&
    revision.importedBy === command.actorUserId &&
    source.revisionId === revision.id &&
    generation.revisionId === revision.id &&
    source.providers.some((item) => item.id === generation.provider.id) &&
    expectedCertification?.revisionId === revision.id &&
    source.questions.length === generation.questions.length &&
    sourceQuestionIds.size === source.questions.length &&
    generationQuestionIds.size === generation.questions.length &&
    [...sourceQuestionIds].every((id) => generationQuestionIds.has(id)) &&
    generation.questions.every(
      (question) =>
        question.revisionId === revision.id &&
        question.certificationId === generation.certification.id &&
        sourceDomains.has(question.domainId) &&
        question.choices.length > 0 &&
        new Set(question.choices.map((choice) => choice.id)).size ===
          question.choices.length &&
        question.correctChoiceIndexes.every(
          (index) => index >= 0 && index < question.choices.length,
        ),
    );
  if (!valid) throw new Error("Import materialization is invalid.");
}

async function loadActiveSources(
  db: Queryable,
): Promise<readonly CatalogRevisionSource[]> {
  const revisions = await db.query<{ revision_id: string; certification_key: string }>(
    `SELECT head.active_revision_id AS revision_id, head.certification_key FROM catalog_heads head ORDER BY head.certification_key ASC`,
  );
  return Promise.all(
    revisions.rows.map(({ revision_id, certification_key }) =>
      loadSource(db, revision_id, certification_key),
    ),
  );
}
...
type CertificationSourceRow = QueryResultRow & {
  id: string;
  revision_id: string;
  provider_id: string;
  external_key: string;
...
type DomainSourceRow = QueryResultRow & {
  id: string;
  revision_id: string;
...
type QuestionSourceRow = QueryResultRow & {
  id: string;
  revision_id: string;
  certification_id: string;
  domain_id: string;
};
type FullSourceRow = QueryResultRow & {
  id: string;
  revision_id: string;
  certification_id: string;
  domain_id: string;
...
async function loadSource(
  ...
  const [providers, certifications, domains, questions] = await Promise.all([
    ...
    db.query<CertificationSourceRow>(
      `SELECT id, revision_id, provider_id, external_key, code, name, total_questions, time_limit_minutes, threshold_numerator, threshold_denominator, scoring_mode FROM certifications WHERE revision_id = $1`,
      [revisionId],
    ),
    db.query<DomainSourceRow>(
      `SELECT id, revision_id, certification_id, name, weight_basis_points, order_index FROM domains WHERE revision_id = $1 ORDER BY order_index`,
      [revisionId],
    ),
    db.query<QuestionSourceRow>(
      `SELECT id, revision_id, certification_id, domain_id FROM questions WHERE revision_id = $1`,
      [revisionId],
    ),
  ]);
  return {
    ...
    domains: domains.rows.map((row) => ({
      ...
      name: row.name,
      weightBasisPoints: row.weight_basis_points,
...
async function loadFullSource(
  ...
  return {
    ...
    certification,
    provider,
    domains: source.domains.filter((item) => item.certificationId === certificationId),
    questions: [...grouped.values()].map((group) => {
      const first = group[0]!;
      return {
        id: first.id,
        ...
        requiredChoiceCount: first.required_choice_count,
        translationStatus: first.translation_status,
      };
    }),
  };
}

function certificationSnapshot(items: readonly PersistedQuestionSnapshot[]): JsonValue {
  const content = items[0]?.content;
  if (!content || typeof content !== "object" || Array.isArray(content)) return {};
  const certification = (content as Record<string, JsonValue>).certification;
...
function object(value: JsonValue | string): JsonValue {
  if (typeof value === "string") return JSON.parse(value) as JsonValue;
  return value;
}
function strings(value: JsonValue | string): string[] {
  const parsed = object(value);
  if (!Array.isArray(parsed) || parsed.some((item) => typeof item !== "string"))
    throw new Error("Stored choice IDs are invalid.");
...
function requiredRow<T extends QueryResultRow, U>(
  ...
  return map(required(result.rows[0], name));
}
