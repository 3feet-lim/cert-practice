...
type ResultFixtures =
  | FixtureSuccess<{ readonly practice: PracticeResultDto; readonly exam: ExamResultDto }>
  | FixtureEmpty<never>
  | FixtureError<never>;
...
type LeaderboardFixtures =
  | FixtureSuccess<LeaderboardDto>
  | FixtureEmpty<LeaderboardDto>
  | FixtureError<never>;
  ...
  return (
    <DataTable
      ...
      columns={[
        ...
        { id: "score", header: "획득 점수", cell: (item) => displayScore(item.earnedScore) },
        {
          id: "review",
          header: "검토 상태",
          cell: (item) => `${item.selectedChoiceIds.length}개 응답 · ${item.correctChoiceIds.length}개 정답`,
        },
      ]}
...
export function StaticPracticeResultScreen({ fixture }: { fixture: ResultFixtures }) {
  if (fixture.state === "empty") {
    return <StatePanel status="empty" title={fixture.title} message={fixture.message} />;
  }
  if (fixture.state === "error") {
    return <StatePanel status="error" title="연습 결과를 열 수 없습니다" message={`${fixture.error.message} ${fixture.error.nextAction}`} />;
  }
  ...
  return (
    <div className="grid max-w-6xl gap-8">
      ...
      <section aria-labelledby="practice-review-title" className="grid gap-4">
        <div>
          <h2 id="practice-review-title" className="text-lg font-bold">문항별 검토</h2>
          <p className="mt-1 text-sm text-muted-foreground">저장된 응답, 정답, 점수와 해설을 기준으로 다시 봅니다.</p>
        </div>
...
export function StaticExamResultScreen({ fixture }: { fixture: ResultFixtures }) {
  ...
  if (fixture.state === "error") {
    return <StatePanel status="error" title="모의고사 결과를 열 수 없습니다" message={`${fixture.error.message} ${fixture.error.nextAction}`} />;
  }
  ...
  return (
    <div className="grid max-w-6xl gap-8">
      <PageHeader
        ...
        title="모의고사 결과"
        description={`${result.certification.code} · ${result.certification.name}`}
        metadata={<Badge tone={result.passed ? "success" : "danger"}>{result.passed ? "합격" : "불합격"}</Badge>}
      />
      <ScoreSummary
        rawScore={displayScore(result.score.rawScore)}
        accuracyRate={displayScore(result.score.accuracyRate, "%")}
      ...
      <dl className="grid gap-4 rounded-xl border border-border bg-card p-5 sm:grid-cols-3">
        <ResultMetadata label="합격 기준" value={`${result.certification.passThreshold}%`} />
        <ResultMetadata label="제출 시각" value={dateTime(result.submittedAt)} />
        <ResultMetadata label="제출 방식" value={result.submissionReason === "expired" ? "시간 만료 자동 제출" : "직접 제출"} />
      </dl>
      <DomainBreakdown
        items={result.domains.map((domain) => ({
          id: domain.domainName,
      ...
      <section aria-labelledby="exam-review-title" className="grid gap-4">
        <div>
          <h2 id="exam-review-title" className="text-lg font-bold">불변 응시 검토</h2>
          <p className="mt-1 text-sm text-muted-foreground">이 응시 당시의 문항 순서, 응답과 채점 결과를 표시합니다.</p>
        </div>
        <ReviewTable questions={result.questions} />
      </section>
    </div>
...
function TrendGraphic({ points }: { points: readonly { accuracyRate: string }[] }) {
  ...
  return (
    <svg viewBox="0 0 320 140" className="h-52 w-full" aria-hidden="true">
      <path d="M 20 120 H 300 M 20 20 V 120" fill="none" stroke="currentColor" opacity="0.25" />
      <polyline points={coordinates} fill="none" stroke="currentColor" strokeWidth="4" />
      {points.map((point, index) => (
        <circle key={index} cx={20 + index * 120} cy={120 - Number(point.accuracyRate)} r="5" fill="currentColor" />
      ))}
    </svg>
  );
}
...
export function StaticHistoryScreen({ fixture }: { fixture: HistoryFixtures }) {
  if (fixture.state === "error") {
    return <StatePanel status="error" title="모의고사 이력을 불러올 수 없습니다" message={`${fixture.error.message} ${fixture.error.nextAction}`} />;
  }
  if (fixture.state === "empty") {
    return <StatePanel status="empty" title={fixture.title} message={`${fixture.message} ${fixture.nextAction}`} />;
  }
  ...
  return (
    <div className="grid max-w-6xl gap-8">
      <PageHeader eyebrow="S8 · HISTORY" title="모의고사 이력" description="연습 결과는 응시 횟수와 점수 추이에서 제외됩니다." />
      <DataTable
        caption="모의고사 응시 이력"
        columns={[
          { id: "submitted", header: "제출 시각", cell: (item) => dateTime(item.submittedAt) },
          { id: "cert", header: "자격증", cell: (item) => item.certificationCode },
          { id: "raw", header: "원점수", cell: (item) => displayScore(item.rawScore) },
          { id: "accuracy", header: "정답률", cell: (item) => displayScore(item.accuracyRate, "%") },
          { id: "pass", header: "결과", cell: (item) => <Badge tone={item.passed ? "success" : "danger"}>{item.passed ? "합격" : "불합격"}</Badge> },
        ]}
      ...
      {trend ? (
        <AccessibleChart
          ...
          rows={trend.points.map((point, index) => ({
            id: point.attemptId,
            cells: [`${index + 1}회`, dateTime(point.submittedAt), displayScore(point.accuracyRate, "%")],
          }))}
        >
...
export function StaticLeaderboardScreen({
  ...
  if (fixture.state === "error") {
    return <StatePanel status="error" title="리더보드를 불러올 수 없습니다" message={`${fixture.error.message} ${fixture.error.nextAction}`} />;
  }
  if (fixture.state === "empty") {
    return <StatePanel status="empty" title={fixture.title} message={`${fixture.message} ${fixture.nextAction}`} />;
  }
  ...
  return (
    <div className="grid max-w-6xl gap-8">
      ...
      <StatusBanner
        title="점수 공개 설정"
        message={privateVisibility ? "현재 점수는 비공개이며 리더보드 후보에서 제외됩니다." : "현재 점수는 공개 상태이며 최고 성과 하나만 순위에 반영됩니다."}
        tone={privateVisibility ? "warning" : "success"}
      >
        <label className="inline-flex items-center gap-2 font-semibold">
          <input type="checkbox" checked={!privateVisibility} readOnly aria-label="점수 공개" />
          점수 공개
        </label>
      </StatusBanner>
      {privateVisibility ? <StatusBanner title="비공개 상태" message="순위는 계속 볼 수 있지만 내 점수와 현재 사용자 표시는 포함되지 않습니다." tone="info" /> : null}
      <DataTable
        caption="공개 최고 성과 리더보드"
        columns={[
          { id: "rank", header: "순위", cell: (item) => `${item.rank}위` },
          { id: "name", header: "사용자", cell: (item) => <span>{item.displayName} {item.isCurrentUser ? <Badge tone="info">나</Badge> : null}</span> },
          { id: "accuracy", header: "정답률", cell: (item) => displayScore(item.accuracyRate, "%") },
          { id: "raw", header: "원점수", cell: (item) => displayScore(item.rawScore) },
          { id: "submitted", header: "대표 응시", cell: (item) => dateTime(item.submittedAt) },
        ]}
        rows={fixture.data.entries.map((entry) => ({ ...entry, id: entry.attemptId }))}
      />
      <p className="text-sm text-muted-foreground">동점 사용자는 같은 순위를 공유하며 다음 순위는 건너뜁니다.</p>
    </div>
  );
}
