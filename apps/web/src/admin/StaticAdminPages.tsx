  ...
  return (
    <section aria-labelledby="pending-users-title" className="space-y-5">
        ...
        <TableBody>
          {users.map((user) => (
            <TableRow key={user.id}>
              ...
              <TableCell>
                <Button disabled aria-label={`${user.displayName} 승인`}>
                  승인
                </Button>
              </TableCell>
            </TableRow>
      ...
      <p className="text-xs text-muted-foreground">
        이 정적 검토 화면의 승인 버튼은 동작하지 않습니다.
      </p>
    </section>
  );
}

export type ImportStaticVariant =
  | "empty"
  | "validating"
  | "valid"
  | "invalid"
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between gap-3">
        <h2 id="import-summary-title" className="text-lg font-bold">
          Dry-run 요약
        </h2>
...
function Dropzone({ disabled = false }: { disabled?: boolean }) {
  return (
    ...
    >
      ...
      <Button className="mt-5" variant="secondary" disabled={disabled}>
        JSON 파일 선택
      </Button>
      <p className="mt-3 text-xs text-muted-foreground">
        이 정적 화면은 파일을 읽거나 업로드하지 않습니다.
      </p>
    </section>
  );
}
  ...
  return (
    <section aria-labelledby="import-page-title" className="space-y-5">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          검증 후에만 카탈로그 교체를 확인합니다.
        </p>
      </div>
      {selected ? (
        <FileSummary
          name={variant === "invalid" ? "broken-dop-c02.json" : "aws-dop-c02.json"}
          size="48.2 KiB / 10 MiB"
          status={
            variant === "validating"
              ? "검증 중"
              : variant === "invalid"
      ...
      {variant === "validating" ? (
        <StatePanel
          status="loading"
          title="JSON을 검증하고 있습니다"
          message="구조, 필수 필드, 도메인 배정 및 번역 요약을 확인하는 모양입니다."
        />
      ) : null}
      {valid ? <ImportSummary validation={valid} /> : null}
      {invalid ? (
        <>
          <ImportSummary validation={invalid} />
          <ValidationErrorList
            errors={invalid.errors.map((error, index) => ({
              id: `${error.code}-${index}`,
      ...
      {variant === "valid" && valid ? (
        <StatePanel
          status="success"
          title="확정 준비 완료"
          message={`검증 토큰은 ${valid.expiresAt ?? "정해진 시각"} 이전에 한 번만 사용할 수 있습니다.`}
          action={<Button disabled>임포트 확정</Button>}
        />
      ) : null}
      {variant === "completed" ? (
        <StatePanel
          status="success"
          title="문제 은행을 교체했습니다"
          message="새 revision이 활성화되었습니다. 이 정적 완료 화면은 실제 데이터를 변경하지 않습니다."
        />
      ) : null}
      {variant === "token-expired" ? (
        <StatePanel
          status="error"
          title="검증 토큰이 만료되었거나 이미 사용되었습니다"
          message="파일 내용이 바뀌었거나 15분 유효 시간이 지났습니다. 같은 파일을 다시 검증한 후 확정하세요."
          action={<Button disabled>다시 검증</Button>}
        />
      ) : null}
      {variant === "commit" && valid ? (
        <StaticDialog
          id="import-commit"
          title="문제 은행을 교체할까요?"
        ...
        >
          <dl className="space-y-2 text-sm">
            <div>
              <dt className="text-muted-foreground">검증 ID</dt>
              <dd className="font-mono">{valid.validationId}</dd>
            </div>
            <div>
              <dt className="text-muted-foreground">토큰 만료</dt>
              <dd>{valid.expiresAt}</dd>
            </div>
          </dl>
          <p className="mt-4 text-xs text-muted-foreground">
            이 dialog의 버튼은 동작하지 않습니다.
          </p>
        </StaticDialog>
      ) : null}
    </section>
